...
Interestingly, in the SCF calculations, the highly-accurate Tran--Wesołowski kinetic functional yields higher errors for all exchange functionals compared to the optimal TF$\lambda$W models. This observation suggests that the TW functional may not be well-suited for the self-consistent densities lacking the shell structure, as it was developed and parametrized for atomic densities exhibiting shell structure~\cite{Tran.2002.IJQC.89.441}. For instance, the MAE and rMAE for the TW functional with the PBE exchange used for the SCF UHF/UGBS densities are 0.07 a.u. and 0.60\%, respectively, while the TF$0.2$W PBE model yields MAE = 2.93 a.u. and rMAE = 8.31\% on the same set of fermionic densities. Clearly, the TW functional is superior for post-SCF calculations, but its performance degrades in self-consistent calculations within the LPS framework.

Table II summarizes the performance of the FA, LDA, and PBE exchange functionals combined with the optimal TF$\lambda$W kinetic functionals for each exchange functional on a set of closed-shell atoms He through Kr. The basis set and numerical grid are the same as in Table I. 

\begin{table}[htb]
\caption{SCF atomic energies (a.u.) for closed-shell atoms from He to Kr calculated using optimal TF$\lambda$W models for each exchange functional identified in Table I. Calculations are done with the same basis set and grid as in Table I. The MAE (a.u.) and rMAE (\%) are with respect to RHF/UGBS values.}
\begin{ruledtabular}
\begin{tabular}{lrrrr}
Atom & TF$\frac{1}{5}$W LDA & TF$\frac{1}{5}$W PBE & TF$\frac{1}{6}$W FA & RHF/UGBS \\[2pt]
\hline\\[-9pt]
He    & $-$2.8184   & $-$2.9571   & $-$3.0195   & $-$2.8617   \\[1.5pt]
Be    & $-$14.4841  & $-$14.8182  & $-$14.6605  & $-$14.5730  \\[1.5pt]
Ne    & $-$128.8016 & $-$129.8300 & $-$128.2917 & $-$128.5471 \\[1.5pt]
Mg    & $-$199.1390 & $-$200.4191 & $-$198.3054 & $-$199.6146 \\[1.5pt]
Ar    & $-$524.7548 & $-$526.8270 & $-$522.9290 & $-$526.8175 \\[1.5pt]
Ca    & $-$674.9299 & $-$677.2756 & $-$672.8084 & $-$676.7582 \\[1.5pt]
Zn    & $-$1776.6511& $-$1780.4172& $-$1773.7276& $-$1777.8481\\[1.5pt]
Kr    & $-$2744.2896& $-$2748.9405& $-$2741.6652& $-$2752.0549\\[1.5pt]
\hline\\[-9pt]
MAE & 1.71 & 1.08 & 3.02 & $-$\\[1.5pt]
rMAE & 0.45 & 0.84 & 1.11 & $-$\\
\end{tabular}
\end{ruledtabular}
\end{table}

In contrast to the results for light atoms H--Ne reported in Table I, where the TF$\frac{1}{6}$W FA model achieved the lowest MAE and rMAE, its accuracy degraded significantly for heavier atoms. Conversely,the TF$\frac{1}{5}$W LDA and TF$\frac{1}{5}$W PBE models exhibit more robust scaling with increasing atomic number, providing lower mean absolute errors (1.71 a.u. and 1.08 a.u., respectively) and significantly reduced relative errors (0.45\% and 0.84\%, respectively) compared to the FA model. This result seems logical since the LDA and PBE exchange functionals are designed to capture the behavior of electrons in a uniform electron gas, which becomes more relevant in larger atoms.
...
Table III summarizes the chemical potentials $\mu$ of the LPS equation~\eqref{eq: LPS Differential Equation} for closed-shell atoms He through Kr calculated using the optimal TF$\lambda$W models for each exchange functional identified in Table I.  
\begin{table}[htb]
\caption{Chemical potentials (a.u.) for closed-shell atoms from He to Kr calculated using optimal TF$\lambda$W models for each exchange functional identified in Table I. The MAE (a.u.) and rMAE (\%) are with respect to RHF/UGBS values.}
\begin{ruledtabular}
\begin{tabular}{lrrrr}
Atom & TF$\frac{1}{5}$W LDA & TF$\frac{1}{5}$W PBE & TF$\frac{1}{6}$W FA & RHF/UGBS \\[2pt]
\hline\\[-9pt]
He    & $-$0.0704   & $-$0.0843   & $-$0.3135   & $-$0.9180   \\[1.5pt]
Be    & $-$0.0737   & $-$0.0854   & $-$0.2724   & $-$0.3093   \\[1.5pt]
Ne    & $-$0.0769   & $-$0.0864   & $-$0.2314   & $-$0.8504   \\[1.5pt]
Mg    & $-$0.0775   & $-$0.0866   & $-$0.2248   & $-$0.2531   \\[1.5pt]
Ar    & $-$0.0785   & $-$0.0869   & $-$0.2119   & $-$0.5910   \\[1.5pt]
Ca    & $-$0.0788   & $-$0.0869   & $-$0.2089   & $-$0.1955   \\[1.5pt]
Zn    & $-$0.0796   & $-$0.0872   & $-$0.1983   & $-$0.2925   \\[1.5pt]
Kr    & $-$0.0800   & $-$0.0873   & $-$0.1941   & $-$0.5242   \\[1.5pt]
\hline\\[-9pt]
MAE & 0.41 & 0.41 & 0.26 & $-$\\[1.5pt]
rMAE & 79.10 & 75.69 & 40.98 & $-$\\
\end{tabular}
\end{ruledtabular}
\end{table}
...
According to the LPS theory, the solution density assymptotically behaves like $n(\textbf{r})\sim \exp{-2\sqrt{-2\mu}r}$. This implies that the bigger magnitude of the chemical potential is associated with a faster decay of the density. To validate this, we examine a self-consistent radial distribution functions for the Be atom in Fig. 1. The optimal TF$\lambda$W kinetic functionals were employed for each exchange functional. All calculations used the basis set with 31 $s$-type basis functions described above. 
\begin{figure}[h]
\includegraphics[scale=0.40]{Figures/be_density_1.png}
\caption{Be SCF radial distribution function $4\pi r^2n(\textbf{r})$ for Fermi--Amaldi, LDA, and PBE exchange functionals combined with TF$\frac{1}{6}$W and TF$\frac{1}{5}$W kinetic energy functionals, respectively. The basis set with 31 $s$-type basis functions was used. The RHF reference is included for comparison.}
\end{figure}

The TF$\frac{1}{6}$W FA density decays more rapidly than either the TF$\frac{1}{5}$W LDA or TF$\frac{1}{5}$W PBE densities. As expected, the choice of exchange functional does not fundamentally alter the density structure, i.e., the FA exchange does not recover atomic shell structure, which remains absent due to the monotonic nature of the TF$\lambda$W kinetic model. The more rapid asymptotic decay observed in the FA density is consistent with established results from statistical atomic theory~\cite{Gombas.2013.statistische}.
